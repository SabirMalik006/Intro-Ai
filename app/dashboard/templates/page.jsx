  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-slate-100">Interview Templates</h1>
          <p className="text-gray-600 dark:text-slate-300 mt-2">Choose from pre-built templates or create your own</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map((category) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-colors cursor-pointer ${selectedCategory === category.id
                ? "bg-gradient-to-r from-teal-600 to-blue-600 text-white"
                : "text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800"
                }`}
            >
              {category.name}
              <span className={`px-2 py-0.5 text-xs rounded-full ${selectedCategory === category.id
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTemplates.map((template) => (
          <div key={template.id} className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-6 hover:shadow-lg transition-shadow">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-slate-100 text-lg">{template.name}</h3>
                <span className={`px-3 py-1 rounded-full text-xs font-medium mt-2 inline-block ${template.category === "technical" ? "bg-blue-50 text-blue-700" :
                  template.category === "behavioral" ? "bg-teal-50 text-teal-700" :
                    template.category === "leadership" ? "bg-purple-50 text-purple-700" :
                      "bg-gray-50 dark:bg-slate-800 text-gray-700 dark:text-slate-200"
                  }`}>
                  {template.category.charAt(0).toUpperCase() + template.category.slice(1)}
                </span>
              </div>
              <button className="p-2 text-gray-400 dark:text-slate-500 hover:text-teal-600 hover:bg-teal-50 rounded-lg">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex gap-2">
              <button className="flex-1 py-2.5 text-sm border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-200 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
                Preview
              </button>
      ...
      <div className="mt-8">
        <div className="bg-gradient-to-r from-teal-50 to-blue-50 border-2 border-dashed border-teal-200 rounded-2xl p-8 text-center">
          <div className="w-16 h-16 bg-white dark:bg-slate-900 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-teal-200">
            <span className="text-3xl text-teal-600">+</span>
          </div>
          <h3 className="text-xl font-semibold text-gray-900 dark:text-slate-100 mb-2">Create Custom Template</h3>
