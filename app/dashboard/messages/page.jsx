  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-slate-100">Messages</h1>
          <p className="text-gray-600 dark:text-slate-300 mt-2">Communicate with candidates and team members</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        <div className="flex flex-col lg:flex-row h-[600px]">
          {/* Conversations List */}
          <div className="lg:w-1/3 border-r border-gray-200 dark:border-slate-700">
            <div className="p-4 border-b border-gray-200 dark:border-slate-700">
              <h3 className="font-semibold text-gray-900 dark:text-slate-100">Conversations</h3>
            </div>

            <div className="divide-y divide-gray-100 dark:divide-slate-700 overflow-y-auto h-[520px]">
              {conversations.map((conversation) => (
                <button
                  key={conversation.id}
                  onClick={() => setActiveChat(conversation.id)}
                  className={`w-full p-4 text-left transition-colors cursor-pointer ${activeChat === conversation.id ? 'bg-teal-50' : 'hover:bg-gray-50 dark:hover:bg-slate-800'
                    }`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <h4 className="font-medium text-gray-900 dark:text-slate-100 truncate">{conversation.name}</h4>
                        <span className="text-xs text-gray-500 dark:text-slate-400">{conversation.time}</span>
                      </div>
                      <p className="text-sm text-gray-600 dark:text-slate-300 truncate mb-1">{conversation.role}</p>
          ...
          <div className="lg:w-2/3 flex flex-col">
            ...
            <div className="p-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 dark:text-slate-100">John Smith</h4>
                  <p className="text-sm text-gray-600 dark:text-slate-300">Frontend Developer • Interview completed</p>
                </div>
              </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto">
              <div className="space-y-4">
                {messages.map((message) => (
                  ...
                  >
                    <div className={`max-w-[70%] rounded-2xl p-4 ${message.sender === 'recruiter'
                      ? 'bg-gradient-to-r from-teal-600 to-blue-600 text-white'
                      : 'bg-gray-100 dark:bg-slate-800 text-gray-900 dark:text-slate-100'
                      }`}>
                      <p>{message.text}</p>
                      <div className={`text-xs mt-2 ${message.sender === 'recruiter' ? 'text-white/70' : 'text-gray-500 dark:text-slate-400'}`}>
                        {message.time}
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-4 border-t border-gray-200 dark:border-slate-700">
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 rounded-xl border border-gray-300 dark:border-slate-600 focus:ring-2 focus:ring-teal-500 focus:border-teal-500 focus:outline-none"
                />
                <button className="px-6 py-3 bg-gradient-to-r from-teal-600 to-blue-600 text-white rounded-xl hover:from-teal-700 hover:to-blue-700 transition-all duration-300 font-medium">
                  Send
                </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        <button className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 hover:border-teal-300 hover:bg-teal-50 transition-all flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="font-medium text-gray-900 dark:text-slate-100">Email Templates</div>
            <div className="text-sm text-gray-600 dark:text-slate-300">Pre-built email responses</div>
          </div>
        </button>
        <button className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 hover:border-teal-300 hover:bg-teal-50 transition-all flex items-center gap-3">
          <div className="w-12 h-12 bg-teal-100 rounded-lg flex items-center justify-center">
          ...
          <div className="text-left">
            <div className="font-medium text-gray-900 dark:text-slate-100">AI Assistant</div>
            <div className="text-sm text-gray-600 dark:text-slate-300">Get AI-powered reply suggestions</div>
          </div>
        </button>
        <button className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 hover:border-teal-300 hover:bg-teal-50 transition-all flex items-center gap-3">
          <div className="w-12 h-12 bg-teal-100 rounded-lg flex items-center justify-center">
          ...
          <div className="text-left">
            <div className="font-medium text-gray-900 dark:text-slate-100">Schedule Messages</div>
            <div className="text-sm text-gray-600 dark:text-slate-300">Schedule follow-ups automatically</div>
          </div>
        </button>
      </div>
    </>
